...
def _load_index():
    ...
    try:
        ...
        for update in reversed(updates):
            ...
            if getattr(msg, "chat", None) and getattr(msg.chat, "id", None) == DB_CHANNEL_ID:
                ...
                if doc:
                    ...
                    if fname == "database.json" and _db_cache is None:
                        ...
                        if result is not None and isinstance(result.get("books"), list):
                            ...
                            if "ai_enabled" in result:
                                with _ai_enabled_lock:
                                    _ai_enabled = bool(result["ai_enabled"])
                            logger.info("Seed database.json loaded ✅ (%d books, %d users)",
                                        len(result.get("books", [])), len(result.get("users", {})))
        ...
        if _db_cache is not None:
            # Save index so future startups are faster
            _save_index()
            return True
        logger.info("No DB found in channel — starting fresh")
    except Exception as e:
