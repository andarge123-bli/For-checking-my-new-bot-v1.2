...
def _load_local_db() -> dict | None:
    """Load database.json from the local filesystem and return parsed data, or None."""
    path = LOCAL_DB_PATH
    ...
    try:
        ...
        if not isinstance(data.get("books"), list):
            logger.warning("Local %s does not contain a valid 'books' list — skipping", path)
            return None
...
def _merge_db(channel_data: dict | None, local_data: dict | None) -> dict:
    """
    Merge channel and local databases.
    Channel data takes precedence for books that already exist there
    (matched by telegram_file_id). Books only in the local file are appended.
    """
    ...
    existing_file_ids = {b.get("telegram_file_id") for b in channel_data.get("books", [])
                         if b.get("telegram_file_id")}
    new_books = [b for b in local_data.get("books", [])
                 if b.get("telegram_file_id") not in existing_file_ids]

    if new_books:
        logger.info("Merging %d new book(s) from local database.json into channel DB", len(new_books))
...
def _load_index():
    ...
    try:
            ...
            if text.startswith("MTU_BOT_INDEX:"):
                ...
                if DB_MSG_IDS.get("db_file"):
                    ...
                    if channel_db is not None:
                        if "ai_enabled" in channel_db:
                            with _ai_enabled_lock:
                                _ai_enabled = bool(channel_db["ai_enabled"])
                        logger.info("DB cache warmed ✅ (%d books, %d users)",
                                    len(channel_db.get("books", [])), len(channel_db.get("users", {})))

                # Merge local database.json (if present) on top of channel data
                local_db = _load_local_db()
                merged = _merge_db(channel_db, local_db)
                _db_cache = merged

                # If merge added new books, push the merged data back to the channel
                channel_book_count = len(channel_db.get("books", [])) if channel_db else 0
                if len(merged.get("books", [])) > channel_book_count:
                    logger.info("Syncing merged DB back to channel (%d books total)...",
        ...
        # No pinned index found — scan recent messages for a database.json or user_choices.json
        # get_updates only returns UNACKNOWLEDGED (pending) updates.
        # We must request channel_post explicitly so Telegram includes them.
        logger.info("No pinned DB index — scanning pending updates for seed database...")
        try:
                ...
                if doc:
                    ...
                    if fname == "database.json" and channel_db is None:
                        ...
                        if result is not None and isinstance(result.get("books"), list):
                            ...
                            logger.info("Seed database.json loaded from channel ✅ (%d books, %d users)",
        ...
        # Merge local file into whatever we got from the channel scan
        local_db = _load_local_db()
        merged = _merge_db(channel_db, local_db)
        ...
        if merged.get("books"):
            ...
            # Save index and push merged DB to channel so future startups are faster
    ...
    # Last-resort: try local file even if channel loading completely failed
    local_db = _load_local_db()
    if local_db:
        _db_cache = local_db
...
def _norm(text: str) -> str:
    """Normalize a faculty/department name for flexible matching.

    Lowercases and removes all non-alphabetic characters so that:
      "Engineering"              → "engineering"
      "Engineering and Technology" → "engineeringandtechnology"
      "️ Public Health"          → "publichealth"  (handles variation selectors)
    """
    return re.sub(r"[^a-z]", "", text.lower())


def _loc_match(stored: str, lookup: str) -> bool:
    """True if stored faculty/department name is compatible with the browse-time lookup.

    Handles two legacy patterns in database.json:
      • Abbreviated faculty  — "Engineering" stored, "Engineering and Technology" looked up
        → stored norm is a prefix of lookup norm
      • Invisible-character dept — "️ Public Health" stored, "Public Health" looked up
        → normalised strings are equal after stripping all non-alpha chars
    Falls back to exact stripped-emoji equality.
    """
    if stored == lookup:
        return True
    ns = _norm(stored)
    ...
    if ns == nl:
        return True
    # Allow stored abbreviation that is a leading fragment of the full name
...
def faculty_keyboard(user_id, prefix="browse"):
    ...
    # "Unordered Upload" button is intentionally NOT shown here in upload mode.
    # Users who want to upload without a course pick Faculty → Dept → Year → Semester
    # and then tap "General/Unordered Files" on the course selection screen.
    markup.add(
...
def delete_custom_course(faculty, dept, year, semester, course_name):
    ...
    # Delete all books in this course (use _loc_match for legacy faculty/dept names)
    db["books"] = [
        b for b in db["books"]
        if not (
...
def upload_course_keyboard(user_id, faculty, dept, year, semester):
    ...
    Shows 'General/Unordered Files' first (so users can skip course assignment),
    then predefined courses from the MTU Course Directory, then user-created custom
    courses, then a 'Create Custom Course' button.
    """
    markup = types.InlineKeyboardMarkup(row_width=1)
    fac_key = strip_emoji(faculty)[:12]
    dept_key = strip_emoji(dept)[:10] if dept else ""
    ...
    # General/unordered option first — user can upload without specifying a course
    markup.add(
        types.InlineKeyboardButton(
            t(user_id, "general_files"),
...
def get_books_for(faculty, dept, year, semester, course=None):
    ...
    Faculty and department matching is flexible (_loc_match) to handle legacy
    database entries that use abbreviated names (e.g. "Engineering" instead of
    "Engineering and Technology") or invisible variation-selector characters.
    """
    db = load_db()
    fac_clean = strip_emoji(faculty)
    dept_clean = strip_emoji(dept) if dept else ""
    ...
    for b in db["books"]:
        ...
        fac_match = _loc_match(b_fac, fac_clean)
        dept_match = _loc_match(b_dept, dept_clean)
        yr_match = b_yr == year
...
def cb_browse_faculty(call):
    ...
    if is_no_semester_faculty(faculty):
        ...
        fac_display = strip_emoji(faculty)
        if not books:
            fac_key_nb = strip_emoji(faculty)[:18]
            no_books_markup = types.InlineKeyboardMarkup(row_width=1)
            no_books_markup.add(
                types.InlineKeyboardButton(
...
def cb_course_general(call):
    ...
    dept_display = strip_emoji(dept) if dept else fac_display
    sem_label = "Semester 1" if semester == "Sem1" else "Semester 2"
    fac_key_local = strip_emoji(faculty)[:12]
    dept_key_local = strip_emoji(dept)[:10] if dept else ""
    yr_key_local = year if year else "direct"
    if not books:
        back_markup = types.InlineKeyboardMarkup(row_width=2)
        back_markup.row(
            types.InlineKeyboardButton(
                t(user_id, "back"),
                callback_data=f"browse_s_{fac_key_local}|{dept_key_local}|{yr_key_local}|{semester}",
            ),
            types.InlineKeyboardButton(t(user_id, "main_menu_btn"), callback_data="main_menu"),
        )
        upload_markup = types.InlineKeyboardMarkup(row_width=1)
        upload_markup.add(
            types.InlineKeyboardButton(
                "📤 Upload to this section",
                callback_data=f"upload_crs_gen_{fac_key_local}|{dept_key_local}|{yr_key_local}|{semester}",
            )
        )
        upload_markup.row(
            types.InlineKeyboardButton(
...
def cb_course_custom(call):
    ...
    dept_key_local = strip_emoji(dept)[:10] if dept else ""
    yr_key_local = year if year else "direct"
    safe_course_local = course_name[:20].replace("|", "-")
    if not books:
        no_books_markup = types.InlineKeyboardMarkup(row_width=1)
        no_books_markup.add(
            types.InlineKeyboardButton(
                "📤 Upload to this course",
                callback_data=f"upload_crs_{fac_key_local}|{dept_key_local}|{yr_key_local}|{semester}|{safe_course_local}",
            )
        )
        no_books_markup.row(
            types.InlineKeyboardButton(
...
def cb_download(call):
    ...
    try:
        ...
        # Only show "help bot" prompt for truly unordered files (no faculty assigned)
...
def _send_help_bot_prompt(user_id, fac_key, dept_key, yr_key, semester):
    ...
    Shows available courses for the same dept/year/semester so the user can tag the file.
    Only called for files that have no faculty set (truly unordered).
    """
    faculty, dept = find_faculty_dept_by_key(fac_key, dept_key)
    year = "" if yr_key == "direct" else yr_key
    ...
    if faculty and semester:
        ...
        for course in all_courses[:12]:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    f"📘 {course}",
                    callback_data=f"hbtag_{fac_key}|{dept_key}|{yr_key}|{semester}|{safe_course}",
                )
            )
...
def cb_help_bot_tag(call):
    """User tapped a course button in the 'Help the Bot' prompt to tag an unordered file."""
    user_id = call.from_user.id
    raw = call.data.replace("hbtag_", "")
    parts = raw.split("|", 4)
    if len(parts) != 5:
        bot.answer_callback_query(call.id, "Invalid data.")
        return
...
def cb_download_by_file_id(call):
    ...
    try:
        ...
        # Only show "help bot" prompt for truly unordered files (no faculty assigned at all)
        if not book.get("faculty") and not book.get("course"):
...
def _process_document(message):
    """Process a single document upload. Called in a thread so multiple files
    sent at once (batch multi-select) are handled in parallel."""
    user_id = message.from_user.id
    state = get_state(user_id)
    action = state.get("action", "")

    if action != "awaiting_file":
        bot.send_message(
            user_id,
            t(user_id, "main_menu"),
            reply_markup=main_menu_keyboard(user_id),
    ...
    try:
        ...
        # Stay in "awaiting_file" so the user can immediately send more files to the same
        # location (batch upload — Telegram delivers each file as a separate message).
        # Show a compact success with a "Done" button to exit.
        done_markup = types.InlineKeyboardMarkup(row_width=1)
        done_markup.add(
            types.InlineKeyboardButton("✅ Done — Main Menu", callback_data="main_menu")
        )
        # Keep state active so the next file (from a multi-select batch) is accepted
        state["action"] = "awaiting_file"
        set_state(user_id, state)
        bot.send_message(
            user_id,
...
def handle_document(message):
    """Spawn a thread per file so multiple files selected at once are processed in parallel."""
    import threading
    threading.Thread(target=_process_document, args=(message,), daemon=True).start()


# ── DB Channel Post Handler ────────────────────────────────────────────────────
# Catches database.json / user_choices.json files posted directly to the DB channel.
# This is the reliable way to detect uploads: get_updates only returns PENDING
# (unacknowledged) updates, so any file uploaded while the bot was already running
# is invisible to get_updates on the next restart. The channel_post handler receives
# those files during the normal polling loop — both live uploads and any queued
# channel_post updates that arrived while the bot was offline.
...
def handle_channel_db_upload(post):
    ...
    if fname == "database.json":
        logger.info("Received database.json upload in DB channel (msg_id=%s) — loading...", post.message_id)
        result = _download_from_channel(doc.file_id)
        ...
        DB_MSG_IDS["db_file"] = doc.file_id
        logger.info("database.json loaded from channel post ✅ (%d books total)", len(merged.get("books", [])))
        # Push merged result back to channel and update index
        _db_executor.submit(_bg_save_db, merged)

    elif fname == "user_choices.json":
        logger.info("Received user_choices.json upload in DB channel (msg_id=%s) — loading...", post.message_id)
        result = _download_from_channel(doc.file_id)
...
def main():
    ...
    if DB_CHANNEL_ID:
        ...
        logger.warning("DB_CHANNEL_ID not set — attempting to load from local database.json only")
        local_db = _load_local_db()
        if local_db:
            with _db_lock:
                _db_cache = local_db
            if "ai_enabled" in local_db:
                with _ai_enabled_lock:
                    _ai_enabled = bool(local_db["ai_enabled"])
            logger.info("Running with local DB only (%d books)", len(local_db.get("books", [])))
        else:
    ...
    # allowed_updates must include 'channel_post' so Telegram delivers
    # documents uploaded directly to the DB channel to our handler.
