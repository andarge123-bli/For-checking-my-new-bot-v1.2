"""
MTU University File-Sharing Telegram Bot — v2.1

New in v2.1
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
NEW FEATURES
  • Language-selection now leads to a short onboarding flow:
      /start → language picker → faculty selection → department selection
    → then main menu.  The chosen faculty/department is saved in the user profile
    and used to send targeted upload notifications.

  • Department upload notifications: when a book is successfully uploaded to a
    specific faculty+department, ALL users whose saved profile matches that
    department receive a short, friendly notification:
        "📚 New file in your department!"
        File name, Year, Semester – nothing else.

  • After creating a new course (browse flow), the user now sees:
        [📤 Upload to this course]  [⬅️ Back]
    instead of being taken straight to the main menu.

  • After downloading an unordered (no-course) file the bot now sends a
    "Help the bot" prompt that lets the user tag the book to their
    faculty → department → semester → course without leaving the conversation.

BUG FIXES (carried forward from v2.0)
  • Semester selection for Freshman in upload flow now works correctly.
  • upload_course_keyboard back button navigates to semester selection (not year).
  • All other v2.0 fixes are preserved.
"""
...
ACTION_ONBOARDING_FAC      = "onboarding_faculty"   # NEW
ACTION_ONBOARDING_DEPT     = "onboarding_dept"       # NEW
...
def cb_download_by_file_id(call):
    ...
    try:
        ...
        # NEW: for unordered books, show tag prompt with faculty→dept selection
        if not book.get("faculty") and not book.get("course"):
            _send_unordered_tag_prompt(user_id)
            return
...
def _send_unordered_tag_prompt(user_id: int) -> None:
    """Ask user to tag an unordered book to a faculty."""
    lang = get_lang(user_id)
    if lang == "am":
        msg = (
            "🆘 *ቦቱን ይርዱ!*\n"
            f"{DIVIDER}\n"
            "ይህ ፋይል ያልተደራጀ ነው። ለየትኛው ፋካልቲ/ዲፓርትመንት ነው?\n"
            "ፋካልቲ ይምረጡ ወይም ዝለሉ።"
        )
    else:
        msg = (
            ...
            f"{DIVIDER}\n"
            "This file has no category yet. Which faculty does it belong to?\n"
...
def cb_untag_faculty(call):
    ...
    # For faculties with no departments, done — just show rating
    if not FACULTIES.get(faculty) or is_special_faculty(faculty):
        bot.answer_callback_query(call.id)
...
def cb_untag_dept(call):
    ...
    lang = get_lang(user_id)
    bot.answer_callback_query(call.id)
    msg = "✅ *Tagged!* Thank you for helping organize! 🌟" if lang == "en" else "✅ *ተለጥፏል!* ስለ ርዳታዎ አመሰግናለሁ! 🌟"
    bot.send_message(user_id, msg, reply_markup=main_menu_keyboard(user_id), parse_mode="Markdown")
...
def cb_upload_faculty(call):
    ...
    if is_no_semester_faculty(faculty):
        ...
        # Freshman and similar: go straight to semester
        bot.send_message(user_id, t(user_id, "select_semester"),
