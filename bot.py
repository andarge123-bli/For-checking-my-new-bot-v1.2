"""
MTU University File-Sharing Telegram Bot — v2.2

New in v2.2
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
NEW FEATURES
  • Unordered books now carry their own faculty / department / year / semester
    metadata.  An "unordered" book is one that has all location fields set but
    has NO course name assigned to it.

  • Upload routing:
      – Books uploaded via the normal faculty → dept → year → semester → course
        flow land in their specific course as before.
      – Books uploaded via the "Upload without course" (formerly "Upload
        Unordered") button now still ask for faculty → dept → year → semester
        and are then stored in the "General / Unordered" slot for that location.

  • After downloading a book that has no course tag the bot presents an
    extended tag-assist flow:
        faculty → department → year → semester → course (or skip at any step)
    so the community can gradually organise uncategorised files.

BUG FIXES
  • Semester selection empty-click bug: after choosing faculty → department →
    year, the "Select Semester" keyboard now always renders with clickable
    Semester 1 / Semester 2 buttons regardless of faculty type.
  • All v2.1 fixes are preserved.
"""
...
PREDEFINED_COURSES: dict[tuple, list[str]] = {
    ...
    ("Engineering and Technology", "Software Engineering", "Year1", "Sem1"): [
    ...
    ("Engineering and Technology", "Software Engineering", "Year1", "Sem2"): [
        "Programming Fundamentals (Python/C)", "Applied Mathematics II", "Discrete Mathematics",
        "Physical Education II", "Technical Writing", "Introduction to Software Engineering",
    ],
    ("Engineering and Technology", "Software Engineering", "Year2", "Sem1"): [
        "Object-Oriented Programming", "Data Structures & Algorithms", "Computer Organization",
        "Probability & Statistics", "Database Systems I", "Software Requirements Engineering",
    ],
    ("Engineering and Technology", "Software Engineering", "Year2", "Sem2"): [
        "Algorithm Design & Analysis", "Operating Systems", "Computer Networks",
        "Database Systems II", "Software Design & Architecture", "Web Technologies",
    ],
    ("Engineering and Technology", "Software Engineering", "Year3", "Sem1"): [
        "Software Construction", "Software Testing & Quality Assurance", "Mobile App Development",
        "Human-Computer Interaction", "Formal Methods", "Numerical Methods",
    ],
    ("Engineering and Technology", "Software Engineering", "Year3", "Sem2"): [
        "Software Project Management", "Software Maintenance & Evolution", "Distributed Systems",
        "Information Security", "Cloud Computing", "Research Methods",
    ],
    ("Engineering and Technology", "Software Engineering", "Year4", "Sem1"): [
        "Senior Project I", "Software Process & Measurement", "Artificial Intelligence",
        "Professional Ethics", "Elective I",
    ],
    ("Engineering and Technology", "Software Engineering", "Year4", "Sem2"): [
    ...
    ("Social Sciences & Humanities", "Marketing Management", "Year2", "Sem1"): [
        ...
        "Statistics", "Financial Accounting", "Sales Management",
    ],
    ("Social Sciences & Humanities", "Marketing Management", "Year2", "Sem2"): [
        "Advertising & Promotion", "Retail Management", "Research Methods",
        "E-Marketing", "Strategic Marketing", "Product Management",
    ],
    ("Social Sciences & Humanities", "Marketing Management", "Year3", "Sem1"): [
        "International Marketing", "Brand Management", "Supply Chain",
        "CRM", "Senior Project I", "Elective I",
    ],
    ...
    ("Social Sciences & Humanities", "Educational Planning & Management", "Year1", "Sem2"): [
        "History of Education", "Introduction to Psychology", "Introduction to Computer",
        "Physical Education II", "Technical Writing", "Philosophy of Education",
    ],
    ("Social Sciences & Humanities", "Educational Planning & Management", "Year2", "Sem1"): [
        "Educational Administration", "Statistics in Education", "Curriculum Development",
        "Educational Measurement & Evaluation", "Research Methods in Education", "Sociology of Education",
    ],
...
TEXTS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "onboarding_done_en": (
            "✅ *Profile saved!*\n"
            "You'll get notified when new files are added to your department. 🔔"
        ),
        "onboarding_done_am": (
            "✅ *መረጃዎ ተቀምጧል!*\n"
...
def onboarding_faculty_keyboard(lang: str) -> types.InlineKeyboardMarkup:
    """Faculty keyboard used during onboarding (different prefix and skip button)."""
    markup = types.InlineKeyboardMarkup(row_width=1)
    seen_keys: set[str] = set()
    for faculty in FACULTIES:
        key = _fac_cb_key(faculty)
...
def onboarding_dept_keyboard(faculty: str, lang: str) -> types.InlineKeyboardMarkup:
    """Department keyboard used during onboarding."""
    markup  = types.InlineKeyboardMarkup(row_width=1)
    ...
    # For Freshman/Remedial (no departments), show a confirm button
...
def _assert_cb_len(cb: str, context: str = "") -> str:
    """Raise immediately if a callback_data string exceeds Telegram's 64-byte limit."""
...
def upload_course_keyboard(user_id: int, faculty: str, dept: str,
    ...
    # FIX: back button goes to semester selection, not year
...
def _notify_department_users(uploader_id: int, fac_clean: str, dept_clean: str,
    ...
    """Notify all users whose profile matches the uploaded department (background thread)."""
    ...
    for uid_str, info in db.get("users", {}).items():
        ...
        if not user_fac or not user_dept:
            continue
        if _loc_match(user_fac, fac_clean) and _loc_match(user_dept, dept_clean):
            # Build a short, clear notification
...
def cb_language(call):
    ...
    state   = get_state(user_id)
    state["lang"]   = lang
    state["action"] = ACTION_ONBOARDING_FAC  # Start onboarding
    set_state(user_id, state)
    db        = load_db()
    ...
    # Greet and immediately start onboarding
...
def cb_onboarding_faculty(call):
    ...
    # For faculties with no departments (Freshman, Remedial), save directly
    if not FACULTIES.get(faculty):
...
def cb_onboarding_dept(call):
    ...
    if not faculty:
        ...
        return
    lang = get_lang(user_id)
    _save_user_profile(user_id, faculty, dept or "")
    state           = get_state(user_id)
    state["action"] = None
    state.pop("ob_faculty", None)
    set_state(user_id, state)
...
def _save_user_profile(user_id: int, faculty: str, dept: str) -> None:
    """Persist faculty and department into the user record in db."""
    db        = load_db()
    user_info = get_user_info(db, user_id)
    fac_clean  = strip_emoji(faculty)
    dept_clean = strip_emoji(dept) if dept else ""
    ...
    save_db(db)
    logger.info("User %s profile saved: fac=%s dept=%s", user_id, fac_clean, dept_clean)
...
def handle_course_name_input(message) -> None:
    ...
    if added:
        # Show options: upload to this course, or go back
        markup = types.InlineKeyboardMarkup(row_width=1)
...
def cb_browse_semester(call):
    ...
    try:
        ...
        if not faculty:
            bot.answer_callback_query(call.id, "Faculty not found.")
            bot.send_message(user_id,
                "⚠️ Could not resolve faculty. Please try again from the main menu.",
                reply_markup=main_menu_keyboard(user_id))
            return
        state = get_state(user_id)
        state["browse_faculty"] = faculty
        ...
        keyboard = course_listing_keyboard(user_id, faculty, dept, year, semester)
        bot.send_message(user_id, header, reply_markup=keyboard, parse_mode="Markdown")
        bot.answer_callback_query(call.id)
    except Exception as e:
        logger.error("cb_browse_semester error: %s", e, exc_info=True)
        bot.answer_callback_query(call.id, "Something went wrong, please try again.")
        try:
            bot.send_message(user_id,
                "⚠️ An error occurred loading courses. Please try again from the main menu.",
                reply_markup=main_menu_keyboard(user_id))
        except Exception:
            pass
...
def _send_unordered_tag_prompt(user_id: int, book: dict | None = None) -> None:
    """Ask user to help tag an unordered book — faculty → dept → year → sem → course."""
    lang = get_lang(user_id)
...
def cb_untag_dept(call):
    ...
    for label, yr in zip(YEARS, YEAR_LABELS):
        markup.add(types.InlineKeyboardButton(
            label, callback_data=f"untag_yr_{fac_key}|{dept_key}|{yr}"
        ))
    skip_label = "⏭️ Skip" if lang == "en" else "⏭️ ዝለል"
    markup.add(types.InlineKeyboardButton(skip_label, callback_data="main_menu"))
...
# ── Help-the-bot prompt (legacy, for ordered books with no course) ────────────
...
def cb_upload_unordered(call):
    ...
    if lang == "am":
        msg = (
            "📂 *ፋይል ያለ ኮርስ ስም ይስቀሉ*\n"
            f"{DIVIDER}\n"
            "ፋካልቲ ይምረጡ — ፋይሉ ያለ ኮርስ ስም ይቀመጣል።"
        )
    else:
        msg = (
            "📂 *Upload Without a Course Name*\n"
            f"{DIVIDER}\n"
            "Select a faculty — the file will be saved without a course name."
        )
...
def cb_upload_faculty(call):
    ...
    if is_no_semester_faculty(faculty):
        ...
        state["action"]          = ACTION_AWAITING_FILE
        state.pop("upload_no_course", None)
        set_state(user_id, state)
        fac_display = strip_emoji(faculty)
        bot.send_message(
            user_id,
...
def cb_upload_semester(call):
    ...
    if not faculty:
        ...
        return
    state = get_state(user_id)
    no_course = state.pop("upload_no_course", False)
    state["upload_faculty"]  = faculty
    state["upload_dept"]     = dept
    state["upload_year"]     = year
...
def cb_upload_back(call):
    ...
    if data == "upload_bk_fac":
        ...
        # FIX: back from course selection → semester selection
    ...
    if data == "admin_list_books":
        ...
        if not users:
            ...
            lines = [
                ...
                f"   🏫 {info.get('faculty','?')} · {info.get('department','?')}"
...
def _process_document(message) -> None:
    ...
    try:
        ...
        # Send department notifications in background if ordered upload
...
def main() -> None:
    ...
    logger.info("Starting MTU Bot v2.1…")
